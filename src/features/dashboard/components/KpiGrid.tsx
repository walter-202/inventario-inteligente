...
export function KpiGrid({ metrics }: { metrics: DashboardMetrics }) {
  const cards = [
    { label: "Ventas últimos 7 días", value: formatearPrecio(metrics.totalSales), icon: BarChart3, tint: colors.accentSoft, iconColor: colors.accentDark },
    { label: "Operaciones de venta", value: String(metrics.salesCount), icon: ShoppingCart, tint: colors.secondarySoft, iconColor: colors.secondaryDark },
    { label: "Unidades en stock", value: String(metrics.stockUnits), icon: Boxes, tint: colors.tertiarySoft, iconColor: colors.tertiaryDark },
    { label: "Productos con stock bajo", value: String(metrics.lowStockCount), icon: CircleAlert, tint: colors.warningSoft, iconColor: colors.warning },
  ];
  return <View style={styles.grid}>{cards.map(({ label, value, icon: Icon, tint, iconColor }) => <Card key={label} mode="outlined" style={styles.card}><Card.Content style={styles.content}><View style={styles.topRow}><View style={[styles.icon, { backgroundColor: tint }]}><Icon size={18} color={iconColor} /></View><Badge style={styles.badge}>Real</Badge></View><Text variant="bodySmall" style={styles.label}>{label}</Text><Text variant="titleLarge" style={styles.value}>{value}</Text></Card.Content></Card>)}</View>;
}
