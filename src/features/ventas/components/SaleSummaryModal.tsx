...
export function SaleSummaryModal({
  ...
  return (
    <AppModalOverlay visible={visible} onDismiss={loading ? () => {} : onDismiss}>
      <Surface style={styles.modal} elevation={4}>
        <Text variant="titleLarge">Confirmar venta</Text>
        ...
        <View style={styles.lines}>
          {items.map((item) => (
            <View key={item.producto.id} style={styles.line}>
              <Text style={styles.lineName}>
                {item.cantidad} × {item.producto.nombre}
              </Text>
              <Text>{formatearPrecio(item.producto.precio * item.cantidad)}</Text>
            </View>
          ))}
        </View>
