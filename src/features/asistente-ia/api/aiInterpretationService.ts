...
export function esConsultaProductoEspecifica(texto: string | null | undefined): texto is string {
  const tokens = (texto ?? "")
    ...
    .replace(/[\u0300-\u036f]/g, "")
    .match(/[a-z0-9]+/g) ?? [];
  return tokens.some((token) => token.length > 1 && !BROAD_PRODUCT_QUERY_TOKENS.has(token));
}
...
{
  "accion": "venta" | "buscar_producto" | "consulta_stock" | "consulta_bajo_stock" | "consulta_ventas" | "registro_producto" | "conversacion" | "desconocida",
...
- Solo estas consultas de lectura están permitidas: buscar un producto por nombre/código (accion="buscar_producto"), stock de un producto nombrado (accion="consulta_stock"), inventario bajo (accion="consulta_bajo_stock") y ventas de HOY (accion="consulta_ventas", consulta.periodo="hoy"). No inventes endpoints, filtros ni operaciones.
- Nunca copies palabras residuales de una pregunta amplia (por ejemplo "qué productos tienen stock") a consulta.producto. Si falta un nombre/código específico, usá accion="desconocida".
